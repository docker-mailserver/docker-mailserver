...
# remove running and stopped test containers
	...
	-@ for container in $$(docker ps -a --filter name='^/mail$$|^ldap_for_mail$$|mail_overri.*|^open-dkim$$|^hadolint$$|^eclint$$|^shellcheck$$' | sed 1d | cut -f 1-1 -d ' '); do docker rm -f $$container; done
