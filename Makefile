...
# remove running and stopped test containers
	...
	-@ for container in $$(docker ps -a --filter ancestor='mailserver-testing:ci' | sed 1d | cut -f 1-1 -d ' '); do docker rm -f $$container; done
...
tests:
	@ NAME=$(NAME) ./test/bats-core/bin/bats --log test/*.bats

.PHONY: ALWAYS_RUN
test/%.bats: ALWAYS_RUN
	@ ./test/bats-core/bin/bats --log $@
