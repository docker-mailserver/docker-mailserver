...
@test "checking amavis: AMAVIS_WIPE_VIRUSMAILS override works as expected" {
  run docker run -ti --rm -e AMAVIS_WIPE_VIRUSMAILS=2 `docker inspect --format '{{ .Config.Image }}' mail` /bin/bash -c 'echo $AMAVIS_WIPE_VIRUSMAILS | grep 2' 
  [ "$status" -eq 0 ]
}
...
@test "checking amavis: old virusmail is wipped by cron" {
  ...
  run docker exec -ti mail bash -c 'find /var/lib/amavis/virusmails/ -type f -mtime +$AMAVIS_WIPE_VIRUSMAILS -delete'
  [ "$status" -eq 0 ]
