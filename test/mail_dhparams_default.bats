...
#   1. The file `ffdhe4096.pem` has not been modified (checksum verification).
#   2. `ffdhe4096.pem` is copied to the configuration directories for postfix and dovecot.
...
function setup_file() {
    local PRIVATE_CONFIG_0
    local PRIVATE_CONFIG_1

    PRIVATE_CONFIG_0="$(duplicate_config_for_container . mail_default_dhparams_both_not_one_dir)"
    docker run -d --name mail_default_dhparams_not_one_dir \
        -v "${PRIVATE_CONFIG_0}:/tmp/docker-mailserver" \
    ...
    PRIVATE_CONFIG_1="$(duplicate_config_for_container . mail_default_dhparams_both_one_dir)"
    docker run -d --name mail_default_dhparams_one_dir \
        -v "${PRIVATE_CONFIG_1}:/tmp/docker-mailserver" \
        -v "$(pwd)/test/test-files:/tmp/docker-mailserver-test:ro" \
        -e DMS_DEBUG=0 \
        -e ONE_DIR=1 \
        -h mail.my-domain.com \
