...
function _setup_default_vars()
{
  ...
  do
    ...
    then
      notify 'err' "Unable to set $var=${DEFAULT_VARS[$var]}"
...
function _setup_dovecot_local_user()
{
  ...
  then
    ...
    # shellcheck disable=SC1003
      ...
      if [[ -f /tmp/docker-mailserver/dovecot-quotas.cf ]]
      then
			  mapfile -t -d ':' user_quota < <(grep "${user}@${domain}:" -i /tmp/docker-mailserver/dovecot-quotas.cf)
...
function _setup_ldap()
{
  ...
  do
    # ! "" AROUND $var - MIGHT BREANK THINGS --------------
    # TODO TEST -------------------------------------------
...
function _setup_ssl()
{
  ...
  case $TLS_LEVEL in
    ...
    "intermediate" )
      ...
      ;;
    * ) notify 'err' 'TLS_LEVEL not found _setup_ssl' ;;
  esac
  ...
  then
    ...
    then
		  ...
		  sed -i "s/\$final_spam_destiny.*=.*$/\$final_spam_destiny = D_BOUNCE;/g" /etc/amavis/conf.d/49-docker-mailserver
			...
			if [[ ${DEFAULT_VARS['EXPLICITLY_DEFINED_SPAMASSASSIN_SPAM_TO_INBOX']} -eq 0 ]]
      then
...
function _setup_logrotate()
{
	...
	case "$LOGROTATE_INTERVAL" in
		...
		"monthly" )
			...
			LOGROTATE="$LOGROTATE  rotate 1\n  monthly\n"
			;;
    * ) notify 'err' 'LOGROTATE_INTERVAL not found in _setup_logrotate' ;;
	esac
...
function _setup_mail_summary()
{
  ...
  case "$PFLOGSUMM_TRIGGER" in
    ...
    "logrotate" )
      ...
      ;;
    * ) notify 'err' 'PFLOGSUMM_TRIGGER not found in _setup_mail_summery' ;;
  esac
}
  ...
  echo "LogFile = /var/log/mail/freshclam.log" >> /etc/logwatch/conf/logfiles/clam-update.conf
	...
	case "$LOGWATCH_INTERVAL" in
		...
		"weekly" )
			...
			;;
    * ) notify 'warn' 'LOGWATCH_INTERVAL not found in _setup_logwatch' ;;
	esac
}
