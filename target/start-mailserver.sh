...
function _setup_dovecot_local_user() {
  ...
  if [ -f /tmp/docker-mailserver/postfix-accounts.cf -a "$ENABLE_LDAP" != 1 ]; then
    ...
    grep -v "^\s*$\|^\s*\#" /tmp/docker-mailserver/postfix-accounts.cf | while IFS=$'|' read login pass; do
...
function _setup_ldap() {
  ...
  for f in /etc/postfix/ldap-users.cf /etc/postfix/ldap-groups.cf /etc/postfix/ldap-aliases.cf /etc/postfix/ldap-domains.cf /etc/postfix/maps/sender_login_maps.ldap; do
...
function _setup_postfix_aliases() {
  ...
  if [ -f /tmp/docker-mailserver/postfix-virtual.cf ]; then
    ...
    (grep -v "^\s*$\|^\s*\#" /tmp/docker-mailserver/postfix-virtual.cf || true) | while read from to; do
      # Setting variables for better readability
