...
function fix
{
  _notify 'info' 'Post-configuration checks'
  for FUNC in "${FUNCS_FIX[@]}"
  do
    ${FUNC}
  done

  _notify 'trace' 'Removing leftover PID files from a stop/start'
...
function _fix_var_amavis_permissions
{
  ...
  _notify 'trace' 'Fixing Amavis permissions'
  chown -hR amavis:amavis "${AMAVIS_STATE_DIR}" || _shutdown 'Failed to fix Amavis permissions'
}

function _fix_cleanup_clamav
{
  _notify 'trace' 'Cleaning up disabled ClamAV'
  rm /etc/logrotate.d/clamav-* /etc/cron.d/clamav-freshclam 2>/dev/null || {
    # show warning only on first container start
    [[ ! -f /CONTAINER_START ]] && _notify 'warn' 'Failed to remove ClamAV configuration'
  }
}

function _fix_cleanup_spamassassin
{
  _notify 'trace' 'Cleaning up disabled SpamAssassin'
  rm /etc/cron.daily/spamassassin 2>/dev/null || {
    # show warning only on first container start
    [[ ! -f /CONTAINER_START ]] && _notify 'warn' 'Failed to remove SpamAssassin configuration'
  }
}
