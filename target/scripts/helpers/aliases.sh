...
function _handle_postfix_virtual_config
{
  ...
  then
    ...
    _notify 'debug' "'/tmp/docker-mailserver/postfix-virtual.cf' not provided - no mail alias/forward created"
  fi
}
...
function _handle_postfix_regexp_config
{
  ...
  then
    _notify 'trace' "Adding regexp alias file postfix-regexp.cf"
...
{
  _notify 'trace' 'Configuring root alias'
  ...
  then
    ...
    _notify 'trace' "'/tmp/docker-mailserver/postfix-aliases.cf' is not provided, it will be auto created."
