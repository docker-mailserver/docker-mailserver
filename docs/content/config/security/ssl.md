...
!!! example
    ...
    Add these additions to the `mailserver` service in your [`compose.yml`][github-file-compose]:
...
5. Your `compose.yml` should include the following:
...
??? tip "Renewing a certificate (Optional)"
    ...
    In the following example, add a new service (`certbot-cloudflare-renew`) into `compose.yml` that will handle certificate renewals:
...
5. Make sure your mount path to the `letsencrypt` certificates directory is correct. Edit your `compose.yml` for the `mailserver` service to have volumes added like below:
...
???+ example "Example: `compose.yml`"

    You should have an existing `compose.yml` with a `mailserver` service. Below are the modifications to add for integrating with `nginx-proxy` and `acme-companion` services:
...
Navigate to that folder and note the 6 character random folder name of the certificate you'd like to use. Then, add the following to your `compose.yml` declaration file:
...
You can also provide your own certificate files. Add these entries to your `compose.yml`:
