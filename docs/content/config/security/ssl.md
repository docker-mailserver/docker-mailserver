...
If for some reason, you can't expose your mail server's port 80 and port 443 (known as the `HTTP-01` challenge), you can consider obtaining a certificate by the `DNS-01` challenge.

First, you need to obtain a Cloudflare API token

1. Login into your Cloudflare dashboard.
2. Navigate to MyProfile -> API Tokens.
3. Create token a using the `Edit zone DNS` template since certbot [requires](https://certbot-dns-cloudflare.readthedocs.io/en/stable/#credentials) `ZONE:DNS:Edit` permission.

!!! Zone resource configuration

    Remember to choose a specific TLD, and create a token that allows access to all zone's resources is insecure here.

4. Create `cloudflare.ini` with the following content:

    ```dosini
    dns_cloudflare_api_token = YOUR_CLOUDFLARE_TOKEN_HERE
    ```

5. Save it to the `.secrets` folder or anywhere you like. (Make sure to restrict its permission with the `chmod 600` command
6. Add the following code into `docker-compose.yml`
    ...
    services:
      mailserver:
        environments:
          ...
          - SSL_TYPE=letsencrypt
        volumes:
          # We need to mount the cert generated by certbot into mail server.
          - ./docker-data/certbot/certs/:/etc/letsencrypt
      certbot-cloudflare:
        ...
        volumes:
          ...
          - ./.secrets:/.secrets
        command: certonly --dns-cloudflare --dns-cloudflare-credentials /.secrets/certbot/cloudflare.ini -d mail.example.com
    ```

    Alternative using the `docker run` command
      ...
      docker run \
        ...
        certonly --dns-cloudflare --dns-cloudflare-credentials /.secrets/certbot/cloudflare.ini -d mail.example.com
      ```

7. Turn it on to generate certificate.

    ```sh
    docker-compose up certbot-cloudflare
    ```

8. You should see the following log output.
...
After complete the steps above, your certificate should be ready to use.
