...
To make use of the web interface you need to setup setup a password otherwise you won't be able to log in.
To do so you must
[set the `password` option for the controller worker to a password hash generated by `rspamadm pw`](#with-the-help-of-a-custom-file).

??? example "Configuration example"
    Below is an example of how the configuration should look like.

    ```bash
    set-option-for-controller password "yourHashedPassword"
    ```
