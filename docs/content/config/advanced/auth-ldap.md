...
- To deliver the emails to different members of Active Directory **Distribution Group** (similar to mailing lists), use a [`user-patches.sh` script](https://docker-mailserver.github.io/docker-mailserver/edge/config/advanced/override-defaults/user-patches/) to modify `ldap-groups.cf` so that it includes `leaf_result_attribute = mail` and `special_result_attribute = member`. This can be achieved simply by:
