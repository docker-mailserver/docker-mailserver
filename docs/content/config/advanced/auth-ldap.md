...
- To deliver the emails to different members of Active Directory **Distribution Group** (in other simple words, mailing lists), editing *ldap-groups.cf* to add `leaf_result_attribute = mail` and `special_result_attribute = member` using a [manually written user-patch](https://docker-mailserver.github.io/docker-mailserver/edge/config/advanced/override-defaults/user-patches/) is necessary. This can be achieved as simple as this:

The configuration shown to get Distribution Group to work is from [here](https://doc.zarafa.com/trunk/Administrator_Manual/en-US/html/_MTAIntegration.html) and [here](https://kb.kopano.io/display/WIKI/Postfix).
