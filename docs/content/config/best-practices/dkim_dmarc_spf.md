...
=== "Rspamd"
    ...
    After running `setup config dkim`, the key files for your primary domain will be located in `/tmp/docker-mailserver/rspamd/dkim/` using the `<domain>-<selector>.private` naming convention (default: `example.com-mail.private`). A basic signing configuration is created in `/tmp/docker-mailserver/rspamd/override.d/dkim_signing.conf`. You can modify this configuration manually if you have advanced needs.

    If you have multiple domains, run `setup config dkim domain <DOMAIN>` for each one to create additional keys. The default config uses a `$domain-$selector` path template, so one `dkim_signing.conf` covers every domain that has a matching key file. Custom selectors are recorded in `/tmp/docker-mailserver/rspamd/dkim_selectors.map`.

    To migrate keys from OpenDKIM, copy `/tmp/docker-mailserver/opendkim/keys/<domain>/<selector>.private` to `/tmp/docker-mailserver/rspamd/dkim/<domain>-<selector>.private`. Keys generated by earlier DMS Rspamd releases were named `<keytype>-<keysize>-<selector>-<domain>.private.txt` (for example `rsa-2048-mail-example.com.private.txt`) and must be renamed to the new layout.
    ...
    ??? warning "Support for DKIM Keys using ED25519"
        ...
        Ed25519 and RSA cannot share a selector (each selector has one DNS TXT record). Generate the RSA fallback with a different selector, then add both keys to a domain-specific `selectors` array in `dkim_signing.conf`:

        ```sh
        setup config dkim
        setup config dkim keytype ed25519 selector mail-ed25519
        ```
        ...
        domain {
            example.com {
                selectors [
                    {
                        path = "/tmp/docker-mailserver/rspamd/dkim/example.com-mail.private";
                        selector = "mail";
                    },
                    {
                        path = "/tmp/docker-mailserver/rspamd/dkim/example.com-mail-ed25519.private";
                        selector = "mail-ed25519";
                    }
                ]
            }
        }
